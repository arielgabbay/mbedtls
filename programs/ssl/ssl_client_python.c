...
#define mbedtls_dont_printf(...) mbedtls_printf(__VA_ARGS__)
...
int pyargs_to_argv(char *** argv, int * argc, PyObject * args) {
	int ret = 1, i;
	char ** argv_alloc = NULL;
	PyObject * arg_lst, * curr_elem, * encoded;
	if (!PyArg_ParseTuple(args, "O", &arg_lst) || !PyList_Check(arg_lst)) {
		goto err;
	}
	(*argc) = PyList_Size(arg_lst) + 1;
	argv_alloc = (char **)malloc(sizeof(char *) * (*argc));
	if (argv_alloc == NULL) {
		goto err;
	}
	argv_alloc[0] = "ssl_client2";
	for (i = 1; i < (*argc); i++) {
		curr_elem = PyList_GetItem(arg_lst, (Py_ssize_t)(i - 1));
		if (!PyUnicode_Check(curr_elem)) {
			goto err;
		}
		encoded = PyUnicode_AsEncodedString(curr_elem, "utf-8", "strict");
		if (encoded == NULL) {
			goto err;
		}
		argv_alloc[i] = PyBytes_AsString(encoded);
		if (argv_alloc[i] == NULL) {
			goto err;
		}
	}
	ret = 0;
...
PyObject * set_opts(PyObject * self, PyObject * args) {
	...
	char *p, *q;
	const int *list;
	if (pyargs_to_argv(&argv, &argc, args)) {
		goto exit;
	}
...
PyObject * init_client(PyObject * self, PyObject * args) {
	(void)self;
	(void)args;
	int ret = 0, i;
#if defined(MBEDTLS_X509_CRT_PARSE_C)
...
{
	int ret = 0, len, tail_len, written, frags, retry_left, real_ret = 0;
	(void)self;
	...
	if (PyArg_ParseTuple(args, "s#", &custom_pms_str, &custom_pms_len)) {
		custom_pms = (unsigned char *)malloc(custom_pms_len);
		if (custom_pms == NULL) {
			ret = 3;
			goto exit;
		}
		memcpy(custom_pms, custom_pms_str, custom_pms_len);
		set_custom_pms(custom_pms, custom_pms_len);
	}
#endif
...
static PyMethodDef methods[] = {
	{"set_opts", (PyCFunction)set_opts, METH_VARARGS,
		"Set argv for client."},
	{"query", (PyCFunction)query, METH_VARARGS,
		"Handshake."},
	{"init_client", (PyCFunction)init_client, METH_VARARGS,
		"Initialize session."},
	{0}
};

static struct PyModuleDef moduleDef = {PyModuleDef_HEAD_INIT, "ssl_client",
	NULL, -1, methods, NULL, NULL, NULL, NULL};

PyMODINIT_FUNC PyInit_ssl_client(void) {
